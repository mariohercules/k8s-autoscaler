...
#### kops edit ig --name $NAME --state --state=s3://$KOPS_STATE_STORE master-us-east-1a 
#### kops edit cluster $NAME --state=s3://$KOPS_STATE_STORE
#### kops update cluster --name $NAME --state s3://$KOPS_STATE_STORE --yes
#### kops rolling-update cluster --name $NAME --state s3://$KOPS_STATE_STORE --yes
